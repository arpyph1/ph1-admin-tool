  ...
  try {
    ...
    for (const page of pages.items) {
      ...
      for (const section of sections) {
        ...
        carouselMatches.forEach((carousel: string) => {
          if (carousel.length > 200) {
            components.push({
              id: `carousel-${components.filter((c: any) => c.id.startsWith('carousel')).length + 1}`,
              type: 'carousel',
              html: carousel,
        ...
        logoGridMatches.forEach((grid: string) => {
          if (grid.includes('img') && grid.length > 150) {
            components.push({
              id: `logo-grid-${components.filter((c: any) => c.id.startsWith('logo-grid')).length + 1}`,
              type: 'logo-grid',
              html: grid,
        ...
        heroMatches.forEach((hero: string) => {
          if (hero.length > 200) {
            components.push({
              id: `hero-${components.filter((c: any) => c.id.startsWith('hero')).length + 1}`,
              type: 'hero',
              html: hero,
        ...
        featureGridMatches.forEach((grid: string) => {
          if (grid.length > 300 && !grid.includes('<img')) { // Not a logo grid
            components.push({
              id: `feature-grid-${components.filter((c: any) => c.id.startsWith('feature-grid')).length + 1}`,
              type: 'feature-grid',
              html: grid,
        ...
        textMatches.forEach((text: string) => {
          if (text.length > 150 && text.length < 1000) {
            components.push({
              id: `text-${components.filter((c: any) => c.id.startsWith('text')).length + 1}`,
              type: 'text-section',
              html: text,
        ...
        ctaMatches.forEach((cta: string) => {
          if (cta.length > 100 && cta.length < 500) {
            components.push({
              id: `cta-${components.filter((c: any) => c.id.startsWith('cta')).length + 1}`,
              type: 'cta',
              html: cta,
    ...
    const unique = components.filter((comp: any, index: number) => {
      return !components.slice(0, index).some((other: any) =>
        other.type === comp.type &&
        Math.abs(other.html.length - comp.html.length) < 50
      );
    });

    const byType = unique.reduce((acc: any, comp: any) => {
